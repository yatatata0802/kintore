...
import { formatRemainingRepsText, formatCompletedRepsText, copyTextToClipboard } from './utils/clipboard';
...
export default function App() {
  ...
  const [isCopiedRemaining, setIsCopiedRemaining] = useState<boolean>(false);
  const [isCopiedCurrent, setIsCopiedCurrent] = useState<boolean>(false);
  const copyRemainingTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const copyCurrentTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const handleCopyRemaining = async () => {
    ...
    setIsCopiedRemaining(true);
    if (copyRemainingTimeoutRef.current) clearTimeout(copyRemainingTimeoutRef.current);
    copyRemainingTimeoutRef.current = setTimeout(() => {
      setIsCopiedRemaining(false);
    }, 2500);

    if (success) {
      showActionToast(`📋 残りをコピー: ${text}`);
    } else {
  ...
  const handleCopyCurrent = async () => {
    const text = formatCompletedRepsText(exercises);
    const success = await copyTextToClipboard(text);
    if (soundEnabled) {
      sounds.playClick();
    }
    setIsCopiedCurrent(true);
    if (copyCurrentTimeoutRef.current) clearTimeout(copyCurrentTimeoutRef.current);
    copyCurrentTimeoutRef.current = setTimeout(() => {
      setIsCopiedCurrent(false);
    }, 2500);

    if (success) {
      showActionToast(`🔥 消化実績をコピー: ${text}`);
    } else {
      showActionToast(`📋 ${text}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col font-sans">
      ...
      <Header
        ...
        isCopiedRemaining={isCopiedRemaining}
        onCopyCurrent={handleCopyCurrent}
        isCopiedCurrent={isCopiedCurrent}
      />
        ...
        {ipadFitMode ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4 items-start">
            ...
            <div className="lg:col-span-6 xl:col-span-6 space-y-2.5">
              ...
              <div className="bg-neutral-900/90 border border-neutral-800 rounded-xl p-2.5 flex items-center justify-between gap-2 shadow-sm flex-wrap">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <button
                    ...
                    className={`px-2 py-1.5 rounded-lg font-bold text-xs shadow-sm flex items-center gap-1 transition-all active:scale-95 border ${
                      isCopiedRemaining
